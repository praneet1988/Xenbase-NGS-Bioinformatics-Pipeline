...
if(($user_input eq "ProcessPublicData")||($user_input eq "--ProcessPublicData"))
{
 my $file=$ARGV[1];
 my $path=$ARGV[2];
 my $sralocation=$ARGV[3];
 &ProcessPublic($file,$path,$sralocation);
}
...
sub ProcessPublic
{
 ...
 my $file=@_[0];chomp $file;my $path=@_[1];chomp $path; my $sralocation=@_[2]; chomp $sralocation;
 if($file eq "")
 {
 ...
 if($sralocation eq "")
 {
  print "\nPlease provide the path to download .sra \.\. You can drag and drop the folder from finder window \.\. Please remove space at the end\n";
  $sralocation=<stdin>;chomp $sralocation;
 }
 print "\nPlease Provide your Linux OS type \:\: Ubuntu or Redhat\n";
 my $OS=<stdin>;chomp $OS;
 if($OS eq "Ubuntu")
 {
  my $counter=0;
  my $path_to_sratoolkit_prefetch=$cwd."\/"."Modules/sratoolkit.2.9.0-ubuntu64/bin/./prefetch";
  my $path_to_sratoolkit_fastqdump=$cwd."\/"."Modules/sratoolkit.2.9.0-ubuntu64/bin/./fastq-dump";
 ...
 {
  my $counter=0;
  my $path_to_sratoolkit_prefetch=$cwd."\/"."Modules/sratoolkit.2.9.0-centos_linux64/bin/./prefetch";
  my $path_to_sratoolkit_fastqdump=$cwd."\/"."Modules/sratoolkit.2.9.0-centos_linux64/bin/./fastq-dump";
  my $username = getpwuid $UID;
  open(F,"$file");
  while(my $data = <F>)
  {
   ...
   else
   {
    ...
    else
    {
     ...
     my $SRA=$SRRid."\."."sra";
     my $current_path_to_srafile=$sralocation."\/".$SRA;
     my $new_path_to_srafile=$path."\/".$samplename."\."."sra";
...
sub interactivesp
{
 ...
 system(sudo,Rscript,$dirtoR,$path,$filename,$colx,$coly,$colenrichment) ==0 or die "\nExcecution Stopped because of errors \.\.\. Contact Praneet Chaturvedi for bugs and issues on github\n";
...
sub ProcessRNASeqSingle
{
 ...
 if($file eq "")
 {
  print "\nPlease provide the path to the Fastq File \.\. You can drag and drop the file from finder window \.\. Please remove space at the end and quotes\n";
  $file=<stdin>;chomp $file;
 }
 if($species eq "")
 {
  print "\nPlease provide the species \.\. Options : human or mouse or xenopus-laevis or xenopus-trop or zebrafish\n";
 ...
 if( $species eq "human")
 {
  ...
  my $path_to_bamtobigwig=$cwd."\/"."Modules"."\/"."Bedgraph_Bigwig_Linux"."\/"."./bedGraphToBigWig";
  my $path_to_chromsize="2.7e9";
  my $genomesize="2.7e9";
  my $start_time=time();
  
  system("$RNASEQbash", "$file", "$quality_encoding", "$bowtie_path", "$rsem_path", "$rsem_index", "$new_dir_for_result", "$fastqc_result_directory", "$path_to_bamtobigwig", "$path_to_chromsize", "$genomesize") ==0 or die "\nExcecution Stopped because of errors \.\.\. Contact Praneet Chaturvedi for bugs and issues on github\n";
 ...
 if($species eq "mouse")
 {
  ...
  my $path_to_bamtobigwig=$cwd."\/"."Modules"."\/"."Bedgraph_Bigwig_Linux"."\/"."./bedGraphToBigWig";
  my $path_to_chromsize="1.87e9";
  my $genomesize="1.87e9";
  my $start_time=time();
...
sub ProcessRNASeqPaired
{
 ...
 if($file1 eq "")
 {
  print "\nPlease provide the path to the Fastq File pair 1 \.\. You can drag and drop the file from finder window \.\. Please remove space at the end and quotes\n";
  $file1=<stdin>;chomp $file1;
 }
 if($file2 eq "")
 {
  print "\nPlease provide the path to the Fastq File pair 2 \.\. You can drag and drop the file from finder window \.\. Please remove space at the end and quotes\n";
  $file2=<stdin>;chomp $file2;
 }
 if($species eq "")
 {
 ...
 if( $species eq "human")
 {
  ...
  system("$RNASEQbash", "$file1", "$file2", "$quality_encoding", "$bowtie_path", "$rsem_path", "$rsem_index", "$new_output_folder", "$fastqc_result_directory", "$path_to_bamtobigwig", "$path_to_chromsize", "$genomesize") ==0 or die "\nExcecution Stopped because of errors \.\.\. Contact Praneet Chaturvedi for bugs and issues on github\n";
...
sub GenerateTPMCounts
{
 ...
 if($Species eq "")
 {
  print "\nPlease provide Species \.\. Options \: human or mouse or xenopus-laevis or xenopus-trop or zebrafish \n";
  $Species=<stdin>;chomp $Species;
 }
 if($output_dir eq "")
 {
...
sub ProcessChIPATACSingleEnd
{
 ...
 if($species eq "")
 {
  print "\nPlease provide the species \.\. Options : human or mouse or xenopus-laevis and xenopus-trop\n";
  $species=<stdin>;chomp $species;
 }
 if($output_folder eq "")
 {
